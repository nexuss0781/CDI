...
> **Scope.** This document describes the implementation evaluated by the CCT-G3.3 submitted run at code revision `4a8402d2e234b4d45a721ac108eda9dab8b56a4e`. It separates the **active CCT language-engine path** from legacy and experimental modules so that a reader can identify the code that produced the recorded CCT evidence. It does not make a fluency, scale, speed, long-context, or production-readiness claim.
...
> **Current engineering state:** CCT-G3.1 established repeated held-out geometry value, CCT-G3.2 established repeated contrast-readout value, CCT-G3.3 established repeated harmonic-memory-band value, and CCT-G3.4 established repeated bounded token-residual value. The selected residual CDI beat GRU in all three frozen-protocol seeds, with a 0.056826 mean validation-loss improvement, but it did not reach the separately pre-registered 2% material-quality margin. All four mechanisms are retained. The result does not authorize scaling, context expansion, capacity changes, or speed claims; any further quality-recovery proposal must be separately pre-registered.
...
This compact fairness design leaves a very small recurrent-core budget. The 48-state CDI structure is exposed through fixed contrasts rather than discarded by mean-only compression, and the fixed contrast readout, sparse geometry, harmonic memory band, and bounded token residual each have bounded three-seed evidence of contribution. The selected residual CDI now clears the per-seed GRU relation, but the requested material-quality margin remains the open CCT question. [2] [4]
...
CCT-G1 established that the repaired active language path can reduce loss on a bounded real-data task. CCT-G2.1 showed stable learning on the full deduplicated pilot corpus and remained within the declared Transformer tolerance, but CDI was above GRU validation loss in all three seeds. CCT-G3.1 showed that sparse geometry improves full CDI against its exact geometry-free counterpart in all three seeds, CCT-G3.2 showed that fixed contrast features improve full CDI against a capacity-matched mean-readout control in all three seeds, CCT-G3.3 showed that the harmonic memory band improves full CDI against a parameter-preserving harmonic-disabled control in all three seeds, and CCT-G3.4 showed that a bounded token residual improves full CDI and beats GRU in all three seeds. The candidate did not reach the separately pre-registered 2% material-quality margin. The governing quality decision is `QUALITY_RECOVERY_PARTIAL`; the 3,000-step rung is not authorized. [8] [9]
