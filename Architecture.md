...
> **Current engineering state:** CCT-G3.1 established repeated held-out geometry value, CCT-G3.2 established repeated contrast-readout value, and CCT-G3.3 established repeated harmonic-memory-band value. Full CDI beat its exact harmonic-disabled counterpart in all three frozen-protocol seeds, with a 0.031414 mean validation-loss improvement. All three mechanisms are retained. Full CDI nevertheless remained above GRU in all three seeds, so the results do not authorize scaling, context expansion, capacity changes, or speed claims; any quality-recovery proposal must be separately pre-registered.
...
This compact fairness design leaves a very small recurrent-core budget. The 48-state CDI structure is exposed through fixed contrasts rather than discarded by mean-only compression, and the fixed contrast readout, sparse geometry, and harmonic memory band each have bounded three-seed evidence of contribution. The CDI-versus-GRU quality relation remains the open CCT question. [2] [4]
