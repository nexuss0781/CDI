...
> **Scope.** This document describes the implementation evaluated through the CCT-G3.6 autonomous bounded continuation at code revision `39f068974cf8297bdb2f54c0921931c0442adbcd`. It separates the **active CCT language-engine path** from legacy and experimental modules so that a reader can identify the code that produced the recorded CCT evidence. It does not make a fluency, scale, speed, long-context, or production-readiness claim.
...
> **Current engineering state:** CCT-G3.1 established repeated held-out geometry value, CCT-G3.2 established repeated contrast-readout value, CCT-G3.3 established repeated harmonic-memory-band value, and CCT-G3.4 established repeated bounded token-residual value. CCT-G3.5 state-conditioned fusion earned `NO_FUSION_EVIDENCE`. The retained residual CDI then passed the autonomous CCT-G3.6 1,500-step continuation: mean validation loss 6.576891, GRU beaten in all three seeds, and the 2% target passed. One separately reviewed next quality rung is eligible; automatic scale, context expansion, capacity changes, and fluency claims remain gated.
...
This compact fairness design leaves a very small recurrent-core budget. The 48-state CDI structure is exposed through fixed contrasts rather than discarded by mean-only compression, and the fixed contrast readout, sparse geometry, harmonic memory band, and bounded token residual each have bounded three-seed evidence of contribution. The selected residual CDI clears the per-seed GRU relation and passed the 2% target at the CCT-G3.6 1,500-step bounded continuation. This supports one separately reviewed next rung, not automatic scale.
...
CCT-G1 established that the repaired active language path can reduce loss on a bounded real-data task. CCT-G2.1 showed stable learning on the full deduplicated pilot corpus and remained within the declared Transformer tolerance, but CDI was above GRU validation loss in all three seeds. CCT-G3.1 showed that sparse geometry improves full CDI against its exact geometry-free counterpart in all three seeds, CCT-G3.2 showed that fixed contrast features improve full CDI against a capacity-matched mean-readout control in all three seeds, CCT-G3.3 showed that the harmonic memory band improves full CDI against a parameter-preserving harmonic-disabled control in all three seeds, and CCT-G3.4 showed that a bounded token residual improves full CDI and beats GRU in all three seeds. CCT-G3.5 fusion did not contribute. CCT-G3.6 showed continued improvement to 6.576891, with CDI beating GRU in all seeds and passing the 2% target. The governing result is `EARNED_BOUNDED_CONTINUATION`; one next rung is eligible for separate review, while the 3,000-step rung remains gated. [8] [9]
