...
> **Review basis:** fresh inspection of `master` at `ae607ed9aa3c09f26c358e6c3187d4fbc83db700`, two focused non-mutating geometry probes, the submitted CCT-G2.1 artifacts, and a full regression run of **246 passing tests**. This file records engineering facts and required work. It does not authorize a training-scale increase or change the CCT decision recorded in `Todo.md`.
...
| Do not use `bash run.sh` for CCT work. | It launches a separate external-production route that is not CCT-governed. |
...
- [ ] CCT-ARCH-001 has a reviewed, pre-registered CCT-G3.1 design.
- [ ] CCT-EMP-002 is encoded in the executable decision report.
- [ ] CCT-RUN-003, CCT-DATA-004, and CCT-DATA-005 are contained before any production/legacy convenience route is used.
- [ ] CCT-TEST-006 adds language-model-level geometry reachability coverage.
- [ ] CCT-ART-008 removes generated writes to tracked `Stages/` files.
- [ ] CCT-DOC-009 makes active versus legacy execution paths unambiguous.
- [ ] P2/P3 issues are scheduled only after the active CCT-G3.1 decision.
