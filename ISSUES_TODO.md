...
> **Review basis:** fresh inspection of the reviewed master revision, two focused non-mutating geometry probes, the submitted CCT-G2.1 and CCT-G3.1 through CCT-G3.4 artifacts, the cumulative remediation regression run, and the CCT-G3.4 local token-residual-control regression run of **288 passing tests**. This file records engineering facts and required work. It does not authorize a training-scale increase or change the CCT decision recorded in `Todo.md`.
...
|---|---|---|
| CCT-ARCH-001 | **Four mechanism effects earned; material-quality gap remains** | CDI has repeated geometry, contrast-readout, harmonic-memory, and token-residual evidence. The selected residual CDI beat GRU in every G3.4 seed with a 0.056826 mean validation-loss improvement (0.8356%), but missed the pre-registered 2% material target by 0.079182. Any further quality-recovery proposal must retain all four mechanisms and be separately pre-registered. |
...
| CCT-PERF-016 | **Partially resolved; empirical optimization deferred** | Immutable topology tensors are cached exactly. Python-token-serial execution remains; CCT-G3.4 does not authorize performance work because the candidate missed the separate 2% material-quality margin despite beating GRU in every seed. |
...
- [x] CCT-ARCH-001 has CCT-G3.1 geometry, CCT-G3.2 readout, CCT-G3.3 harmonic, and CCT-G3.4 token-residual three-seed evidence; the 2% material-quality margin remains the active boundary.
